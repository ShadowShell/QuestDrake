...
private:
	...
	//CodeChange - 0.0.3
	void StateDoubleQuickInput( CPlayer& player, const SInputEventData& inputEvent );
	void ProcessDoubleQuick( CPlayer& player, const SPlayerPrePhysicsData& prePhysicsEvent );
...
const CPlayerStateMovement::TStateIndex CPlayerStateMovement::GroundMovement( CPlayer& player, const SStateEvent& event )
{
	...
	case STATE_EVENT_EXIT:
		...
		m_flags.ClearFlags( EPlayerStateFlags_DoubleQuick );
		break;
	...
	case PLAYER_EVENT_PREPHYSICSUPDATE:
		{
			...
			ProcessDoubleQuick( player, prePhysicsEvent );
...
const CPlayerStateMovement::TStateIndex CPlayerStateMovement::StateGroundInput( CPlayer& player, const SInputEventData& inputEvent )
{
	...
	case SInputEventData::EInputEvent_Sprint:
		...
		break;
	case SInputEventData::EInputEvent_DoubleQuick:
		StateDoubleQuickInput( player, inputEvent );
		break;
	}
...
//CodeChange - 0.0.3
void CPlayerStateMovement::StateDoubleQuickInput( CPlayer& player, const SInputEventData& inputEvent )
{
	CRY_ASSERT( inputEvent.m_inputEvent == SInputEventData::EInputEvent_DoubleQuick );

	inputEvent.m_activationMode == eAAM_OnPress ? m_flags.AddFlags( EPlayerStateFlags_DoubleQuickPressed ) : m_flags.ClearFlags( EPlayerStateFlags_DoubleQuickPressed );
}

void CPlayerStateMovement::ProcessDoubleQuick( CPlayer& player, const SPlayerPrePhysicsData& prePhysicsEvent )
{
	uint32 flagsToCheck = g_pGameCVars->cl_sprintToggle ? (EPlayerStateFlags_DoubleQuickPressed | EPlayerStateFlags_DoubleQuick) : (EPlayerStateFlags_DoubleQuickPressed);

	if( m_flags.AreAnyFlagsActive( flagsToCheck ) && CPlayerStateUtil::ShouldSprint( player, prePhysicsEvent.m_movement, player.GetCurrentItem() ) )
	{
		if (!player.IsDoubleQuicking())
		{
			// notify IPlayerEventListener about sprinting (just once)
			OnSpecialMove(player, IPlayerEventListener::eSM_SpeedDoubleQuick);
		}

		m_flags.AddFlags( EPlayerStateFlags_DoubleQuick );
		if( player.GetPlayerInput()->GetType() == IPlayerInput::PLAYER_INPUT )
		{
			static_cast<CPlayerInput*> (player.GetPlayerInput())->ClearCrouchAction();
			player.SetStance(STANCE_STAND);
