...
void CPlayer::UpdateAnimationState(const SActorFrameMovementParams &frameMovementParams)
{
	...
	else if (IsPlayer() && pActionController)
	{
		...
		//CodeChange - 0.0.3
		TagID movementTag = TAG_ID_INVALID;
		if(IsSprinting())
			movementTag = PlayerMannequin.tagIDs.sprint; 
		else if(IsDoubleQuicking())
			movementTag = PlayerMannequin.tagIDs.doubleQuick;
...
//CodeChange - 0.0.3
bool CPlayer::IsDoubleQuicking() const
{
	return (IsPlayer() && m_stateMachineMovement.StateMachineActiveFlag( EPlayerStateFlags_DoubleQuick ) );
}
