...
{
	OnActionDoubleQuick(entityId,actionId,activationMode,value);
...
//CodeChange - 0.0.3
bool CPlayerInput::OnActionDoubleQuick(EntityId entityId, const ActionId& actionId, int activationMode, float value)
{
	m_doubleQuickButtonDown = (activationMode != eAAM_OnRelease);
	SInputEventData inputEventData( SInputEventData::EInputEvent_DoubleQuick, entityId, actionId, activationMode, value );
	m_pPlayer->StateMachineHandleEventMovement( SStateEventPlayerInput( &inputEventData ) );
