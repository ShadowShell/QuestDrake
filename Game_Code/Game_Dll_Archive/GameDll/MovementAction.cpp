...
const float MAX_AIM_ANGLE				= gf_PI*		GetExposed(1);
const bool SPIN_WITH_ENTITY				= false;
const float TRAVEL_ANGLE_BLEND_RATE		=				GetExposed(2);
const float MOVE_SPEED_BLEND_RATE		=				GetExposed(3);
const float SPIN_FACTOR					=				GetExposed(4);
const float TURNBIAS_MIN_SPEED			=				GetExposed(5);
const float TURNBIAS_MAX_SPEED			=				GetExposed(6);
const float TURNBIAS_MIN_ANGLE			= gf_PI/		GetExposed(7);
const float TURNBIAS_MAX_ANGLE			= gf_PI;
	...
	if (notInAir)
	{
		...
		else
		{
			if (!player.IsThirdPerson())
			{
				...
				const float MIN_STRAFE_SPEED = GetExposed(8);
				const float MIN_STRAFE_SPEED_MOVING = GetExposed(9);
			...
			else
			{
				...
				const float TARGET_MOVING_DP = GetExposed(10);
				...
				//const float TRIGGER_TURN_ANGLE			= DEG2RAD(75.0f);
				...
				const float TRIGGER_TURN_ANGLE			= DEG2RAD(GetExposed(11));
				const float MIN_TURN_ANGLE				= DEG2RAD(GetExposed(12));
				const float MAX_TURN_ANGLE				= DEG2RAD(GetExposed(13));
				const float MIN_SPIN_ANGLE				= DEG2RAD(GetExposed(14));
