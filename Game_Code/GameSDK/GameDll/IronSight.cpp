...
: m_pWeapon(0), 
m_savedFoVScale(0.000f),
m_zoomed(false),
m_zoomingIn(false),
m_zoomTimer(0.000f),
m_zoomTime(0.000f),
m_startZoomFoVScale(0.000f),
m_endZoomFoVScale(0.000f),
m_currentZoomFoVScale(1.000f),
m_currentZoomBonusScale(1.000f),
m_smooth(false),
m_enabled(true),
m_currentStep(0),
m_initialNearFov(gEnv->bMultiplayer ? 60.000f : 55.000f),
m_averageDoF(50.000f),
m_swayTime(0.000f),
m_lastRecoil(0.000f),
m_leaveZoomTimeDelay(0.000f),
...
m_activated(false),
//CodeChange - 0.0.2
worZoomFactor(0.000f),
worZoomingOut(false),
isStabilizing(true),
isDestabilizing(false)
{
	m_canZoom = new bool;
	*m_canZoom = true;
...
Ang3 CIronSight::ZoomSway(CPlayer& clientPlayer, float time)
{
	...
	{
		m_swayTime = m_zoomParams->zoomSway.stabilizeTime * (1.000f - m_zoomParams->zoomSway.scaleAfterFiring);
	}
	...
	// Time factor
	float minScale = minScale = m_stable ? m_zoomParams->zoomSway.holdBreathScale : m_zoomParams->zoomSway.minScale;
	float factor = minScale;
	float settleTime = 0.0f;
	float holdTime = m_zoomParams->zoomSway.holdBreathTime;

	if(!isDestabilizing)
		settleTime = m_zoomParams->zoomSway.stabilizeTime;
	else
		settleTime = m_zoomParams->zoomSway.destabilizeTime;

	if(!isStabilizing && m_swayTime <= holdTime && !isDestabilizing)
		m_swayTime += time;
	else if(m_swayTime < settleTime)
	{
	...
	if(!isDestabilizing)
		swayAmount = factor * stanceScale;
	else
		swayAmount = 1 - (factor * stanceScale);
	...
	if(m_swayTime >= settleTime && isStabilizing)
	{
		m_swayTime = 0.0f;
		isStabilizing = false;
	}
	//CodeChange - 0.0.2 - Fancies Work

	if (m_zoomOutScheduled || worZoomingOut) // smooth zoom out
	{
		worZoomingOut = true;
		if (worZoomFactor > 0.000f)
		{
			worZoomFactor -= time;
			if (worZoomFactor < 0.0001f)
			{
				worZoomFactor = 0.0001f;
			}
			swayAmount = (swayAmount / 0.990f)*worZoomFactor;
		}
	}
	else if (worZoomFactor <= 1) // smooth zoom in
	{
		worZoomFactor += time;
		if (worZoomFactor > 1.000f)
		{
			worZoomFactor = 1.000f;
		}
		swayAmount = (swayAmount / 0.990f)*worZoomFactor;
	}
	//end Fancies Work
	...
	return Ang3(x, 0.000f, y);
}

//CodeChange - 0.0.2
void CIronSight::setCanZoom(bool isZoomable)
{
	*m_canZoom = isZoomable;
}
